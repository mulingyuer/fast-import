...
Fast Import 是一款专为提升开发效率量身打造的 VS Code 插件。它基于 TypeScript AST（抽象语法树）解析技术，通过智能的光标定位与自动化重构功能，极大地减少了在编写 `import` / `export` 语句和对象解构时的琐碎光标移动，让您的编码流程更加顺畅。
...
### 2. 智能 Export 定位 (Alt + I)

在编写 `export` 具名导出语句时，插件同样能够智能定位光标至 `{}` 内部，并自动触发代码补全提示。

- **本地具名导出**：`export { | }`。
- **重导出**：`export { | } from "module"`。
- **类型重导出**：`export type { | } from "module"`。

插件全面支持多行 `export` 语句，无论光标位于该语句的何处，均能准确识别。
...
| 功能 | 快捷键 | 备注 |
| :--- | :--- | :--- |
| **智能定位 / 转换** | `Alt + I` | 核心功能，支持 Import、Export、解构及自动转换 |
